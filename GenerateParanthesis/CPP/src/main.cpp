...
public:
    ...
    void to_json(json& j) {
        j = json{{"input", input}, {"output", output}, {"result", result}}; // Serialize input and output
    }
};
...
public:
    ...
    std::vector<TestCase> readTestcases() {
        ...
        for (int i = 0; i < j.size(); i++) {
            TestCase testcase;
            testcase.input = j[i]["input"].get<int>(); // Read input as an integer
            testcase.output = j[i]["output"].get<std::vector<std::string>>(); // Read output as a vector of strings
...
int main() {
    Util util;
    GenerateParanthesisTester tester; 
    std::vector<TestCase>& testcases = util.testcases;
    bool failed = false;
    for (TestCase& testcase : testcases) {
        bool result = tester.test(testcase.input, testcase.output); // Call the test function
        testcase.result = result; // Set result based on the test function's return value
        if (!testcase.result) {
            failed = true;
        }
    }
